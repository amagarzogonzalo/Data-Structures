...
//
// COSTE: O(n), donde n es el número de nodos del árbol de entrada; se recorrerá cada nodo necesariamente una vez
// 
int nodo_cuadrado_mayor_prof(const BinTree<int> &tree, int & prof, int &sumahijos, int& prof_sol, int &sol) {
	if (tree.empty()) {
		prof = 0;
		sumahijos = 0;
		return sol;
		...
		int profI, profD, sumahijosI, sumahijosD;
		profI = prof + 1;
		profD = prof + 1;
		int nodoDer = nodo_cuadrado_mayor_prof(tree.right(), profD, sumahijosD, prof_sol, sol);
		int nodoIz = nodo_cuadrado_mayor_prof(tree.left(), profI, sumahijosI, prof_sol, sol);
		sumahijos = sumahijosI + sumahijosD;
		if (sumahijos == tree.root() && prof >= prof_sol) {
			prof_sol = prof;
			sol = tree.root();
 		}
		else {
			if (nodoIz != -1) sol = nodoIz;
			else if (nodoDer != -1) sol = nodoDer;
		}
		sumahijos = sumahijos + tree.root();
		return sol;
	}
}
...
void tratar_caso() {
	...
	int prof = 0, sumahijos = 0, prof_sol = 1, sol = -1;
	std::cout << nodo_cuadrado_mayor_prof(tree, prof, sumahijos, prof_sol, sol) << std::endl;
}
