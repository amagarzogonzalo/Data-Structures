...
private:
	...
	int in_interval(const T &lower, const T &upper, Node* node) const;
};
...
int SetTree<T>::elems_in_interval(const T &lower, const T &upper) const {
	...
	if (this->root_node != nullptr) return in_interval(lower, upper, this->root_node);
	else return 0;
}
template <typename T>
int SetTree<T>::in_interval(const T &lower, const T &upper, Node* node) const {
	if (node->left == nullptr && node->right == nullptr)
		if (node->elem >= lower && node->elem <= upper) return 1;
		else return 0;
	else {
		int cont = 0;
		int left = 0;
		int right = 0;
		if (node->left != nullptr && node->elem > lower)
			left = in_interval(lower, upper, node->left);
		if (node->right != nullptr && node->elem < upper)
			right = in_interval(lower, upper, node->right);
		if (node->elem >= lower && node->elem <= upper) cont = left + right + 1;
