...
template <typename T>
int gen(const BinTree<T>& b, bool& g) {
	if (b.empty()) {
		return 0;
	}
	else {
		int ai = gen(b.left(), g);
		int ad = gen(b.right(), g);
		if (!b.left().empty() && (b.root() <= (b.left().root() + 17))) g = false;
		if (!b.right().empty() && (b.root() <= (b.right().root() + 18))) g = false;
		if (!b.left().empty() && !b.right().empty() && (b.left().root() <= (b.right().root() + 1))) g = false;
		if (b.left().empty() && !b.right().empty()) g = false;
