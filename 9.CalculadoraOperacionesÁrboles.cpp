...
void mostrar( MapTree <string, int, OrdenLexicografico> sol) {
...
int resolver_caso(BinTree<string> tree, MapTree <string, int, OrdenLexicografico> & sol) {
	if (tree.left().empty() && tree.right().empty()) {
		if(sol.contains(tree.root())) return sol.at(tree.root());
		else return std::stoi(tree.root());
	}
	else {
		if (tree.root() == "+") {
			int a = resolver_caso(tree.left(), sol);
			int b = resolver_caso(tree.right(), sol);
			return a + b;
		}
		if (tree.root() == "-") {
			int a = resolver_caso(tree.left(), sol);
...
bool tratar_caso() {
	...
	cin >> n;
	if (n == 0) return false;
	MapTree <string, int, OrdenLexicografico> sol;
	while (n > 0) {
		BinTree <string> tree = parse(cin);
		if (sol.contains(tree.left().root())) sol.at(tree.left().root()) = resolver_caso(tree.right(),sol);
		else sol.insert({ tree.left().root(),resolver_caso(tree.right(),sol)});
		n--;
	}
	mostrar(sol);
	return true;
